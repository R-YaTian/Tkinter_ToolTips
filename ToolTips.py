...
class ToolTips:
    ...
    def showToolTips(self, event):
        ...
        if (widget_ref == None or widget_ref not in self.widgets):
            ...
            print(
                "The hovered widget is not in the list of widgets provided to this instance.")
            return(False)
        ...
        try:
            ...
            print(
                "An error occured while trying to find the tooltip text for the hovered widget.")
            return(False)
        ...
        w_master = top_master.nametowidget(
            widget_ref.winfo_parent())  # event widget's master
            ...
            if (min_x + self.tt_width > tm_bbox[2]):
                ...
                if (max_x - self.tt_width >= 0):
                    ...
                    increment = math.floor(
                        tt_max_width / math.ceil(tt_char_width))
        ...
        if (self.tt_widget == None):
            ...
            #		The correct text will be added shortly below.
                        self.tt_widget = ttk.Label(top_master, text=self.tt_text.replace(
                            "\n", " "), background=self.bg_color_, foreground=self.fg_color_, font=self.tt_font, relief='ridge', borderwidth=0)
        else:
